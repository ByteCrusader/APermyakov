...
/**
 * Class for modulate role repository.
 *
 * @author apermyakov
 * @version 1.0
 * @since 20.12.2017
 */
public class RoleRepository implements RepositoryInterface<TransferObject> {
     ...
     */
    private PsqlWorker worker = new PsqlWorker();
    ...
    public void add(TransferObject object) {
        try (Connection connection = this.connector.getConnection()) {
            worker.add(connection, "role", "name", object.getRole());
    ...
    public void update(TransferObject object) {
        try (Connection connection = this.connector.getConnection()) {
            worker.update(connection, "role", "name", object.getRole(), object);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    public void remove(TransferObject object) {
        try (Connection connection = this.connector.getConnection()) {
            worker.remove(connection, "role", "name", object.getRole());
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }

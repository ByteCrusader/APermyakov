...
/**
 * Filter for check role of user when put.
 *
 * @author apermyakov
 * @version 1.0
 * @since 11.12.2017
 */
public class PutRoleFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (("administrator").equals(role)) {
            response.sendRedirect(request.getContextPath() + "/admin/put");
        } else {
            response.sendRedirect(request.getContextPath() + "/user/put");
        }
    }
